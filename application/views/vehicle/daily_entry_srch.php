<div class="table-responsive">
	<table class="table table-bordered table-striped table-condensed mb-none">
		...
		<tbody>
			...
			{
				echo "<tr><td colspan='21' class='cen no-data-found'>Record Not Found</td></tr>";
				die();
			}
			...
			foreach ($vrun_data as $key => $value) { 
				...
				$balamt_data=$this->CrudModel->select_data("select sum(case when acc_trantype =2 then acc_amt else 0 end) as dr_amt from account where acc_sourceid=? and acc_vochfor=3 group by acc_sourceid",array($value['vrun_id']));

				$bill_amt=round($value['vrun_fareamt'],2);
				$paid_amt=0;
				if(count($balamt_data) > 0 && $balamt_data[0]['dr_amt']!=null)
					$paid_amt=round($balamt_data[0]['dr_amt'],2);
			...
			<tr id="row_<?php echo $value['vrun_id']; ?>">
				...
				<td class="h cen" style="vertical-align: middle;">
                 	<?php if($bal > 0) { ?>
                    <a class="mb-xs mt-xs mr-xs btn btn-xs  btn-primary" onclick="billSattlement('<?php echo $value['vrun_id']; ?>')" title="Click Settle Bill" data-toggle="modal" data-target="#addamt_modal">

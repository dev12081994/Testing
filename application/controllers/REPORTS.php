...
class Reports extends CI_Controller
{
    ...
    {	
		if($this->ion_auth->logged_in())
		{
            ...
            $inventory_data=$this->CrudModel->select_data("select inv_paidstatus,inv_billdisc,inv_date,inv_for,inv_id,inv_docs,inv_type,CONCAT_WS(' ',m.c_firstname,m.c_middlename,m.c_lastname) as vendor,inv_billno,stk_qty,stk_gstinclusive as inclusive,stk_rpu,stk_disc,stk_disctype,stk_gstper,stk_mfdt,stk_expdt,prod_name,prod_unit from s_stock st
				...
				where stk_status=1 and inv_status=1 $qry_con order by inv_id desc",$qry_param);
            ...
            <tr>
                ...
                <td class="text-right"><?php if($billdisc>0)echo $billdisc; ?></td>
            ...
            <tr>
                ...
                <th colspan="1" class="text-right"><?php echo $tot_billdisc; ?></th>
		...
		if($this->ion_auth->logged_in())
		{
            ...
            {                
                ...
                {                    
                    ...
                    array_push($qry_param,$_REQUEST['srch_memtype']-1); 
                }
                else
                {                    
                    $qry_con .=" and m.c_type  = ? and inv_type  = ?";
                    array_push($qry_param,$_REQUEST['srch_memtype'],2); 
                }                                
            }
	...
	{ 	
		if($this->ion_auth->logged_in())
		{
            ...
            $data['inventory_data']=$this->CrudModel->select_data("select inv_date,inv_for,inv_perticular,inv_id,inv_docs,inv_type,(inv_billdisc-inv_roundoff)as  inv_billdisc,inv_paidstatus,CONCAT_WS(' ',m.c_firstname,m.c_middlename,m.c_lastname) as vendor,inv_billno,sl_qty,sl_gstinclusive as inclusive,sl_rpu,sl_disc,sl_disctype,sl_gstper,inv_paidstatus,sl_mfdt,sl_expdt,prod_name,prod_unit,prod_id from s_sale sl
				left join s_invoice s on s.inv_id=sl.sl_invid 
				left join mas_members m on m.c_id=s.inv_perticular 
				left join mas_product pm on pm.prod_id=sl.sl_prodid
	...
	{ 	
		if($this->ion_auth->logged_in())
		{
			...
			$data['vehicle_data']=$this->CrudModel->select_data("select v.vrun_date,mvt.vt_name,mv.v_num,concat_ws(' ',dmm.c_firstname,dmm.c_middlename,dmm.c_lastname) as driver,v.vrun_runstatus,v.vrun_meterstart,v.vrun_meterstop,concat_ws(' ',cmm.c_firstname,cmm.c_middlename,cmm.c_lastname) as customer,v.vrun_from,v.vrun_to,v.vrun_work,v.vrun_fareamt,v.vrun_remark,v.vrun_docs,v.vrun_id,
                (select sum(acc_amt) from account where acc_status =1 and acc_vochfor=3 and acc_voucher=3 and acc_sourceid=mv.v_id and acc_trandt =v.vrun_date) as expense
             from vehicle v 
				left join mas_vehicle mv on mv.v_id=v.vrun_vid
				left join mas_vehicletype mvt on mvt.vt_id=mv.v_typeid
				left join mas_members cmm on cmm.c_id=v.vrun_memid
    ...
    {   
        if($this->ion_auth->logged_in())
        {
            ...
            //place where the excel file is created  
            $file_name = "VehicleExpenseReport_csv";         
            $myFile='uploads/'.$file_name.'.xls';
            //pass retrieved data into template and return as a string  
        ...
        if($this->ion_auth->logged_in())
        {
            ...
            else
            {
                ...
                <tr id='row_<?php echo $value["prod_id"]; ?>'>
                    ...
                    <td><?php echo $value['prod_remark']; ?></td>  
                </tr> 
            <?php } }
        }
        else
    ...
    {   
        if($this->ion_auth->logged_in())
        {
            ...
            $data['inventory_data']=$this->CrudModel->select_data("select f_name,f_logo,f_contact,f_email,f_address,inv_date,inv_id,inv_for,inv_disc,inv_perticular,inv_id,inv_docs,inv_type,inv_billno,inv_transportcharge,inv_roundoff,sl_qty,sl_rpu,sl_disc,sl_disctype,sl_gstper,sl_mfdt,sl_expdt,prod_name,prod_unit from s_sale sl
                left join s_invoice s on s.inv_id=sl.sl_invid 
                left join mas_product pm on pm.prod_id=sl.sl_prodid
    ...
    public function srchMemReportOld()
    {   
        if($this->ion_auth->logged_in())
        {
            $qry_con='';
            ...
            {
                $ldg_data=$this->CrudModel->select_data("
                select acc_id,'invoice' as location,acc_trantype,sl_disctype as disctype,inv_billdisc,a.acc_trandt as date,sl_invid as inv_id,inv_remark as remark,p.prod_name as descri,sl_qty as qty,p.prod_unit as unit,sl_rpu as rate,sl_gstper as gstper,sl_disc as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                ...
                UNION ALL
                select acc_id,'account' as location,acc_trantype,'' as disctype,0 as inv_billdisc,a.acc_trandt as date,'' as inv_id,acc_remark as remark,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ) as descri,1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
            ...
            {
                $ldg_data=$this->CrudModel->select_data("
                select acc_id,'invoice' as location,acc_trantype,'' as disctype,inv_billdisc,a.acc_trandt as date,'' as inv_id,vrun_remark remark,v.vrun_work as descri,1 as qty,'' as unit,vrun_fareamt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                ...
                UNION ALL
                select acc_id,'account' as location,acc_trantype,'' as disctype,0 as inv_billdisc,a.acc_trandt as date,'' as inv_id,acc_remark as remark,'Expense On Vehicle' as descri,1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
            ...
            {
                $ldg_data=$this->CrudModel->select_data("
                select acc_id,'invoice' as location,acc_trantype,stk_disctype as disctype,inv_billdisc,a.acc_trandt as date,stk_invid as inv_id,acc_remark as remark,p.prod_name as descri,stk_qty as qty,p.prod_unit as unit,stk_rpu as rate,stk_gstper as gstper,stk_disc as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join s_invoice i on i.inv_id = a.acc_sourceid
                ...
                left join s_sale s on s.sl_invid= i.inv_id
                left join mas_product p on p.prod_id = s.sl_prodid                 
                where sl_status=1 and acc_vochfor=2 and acc_voucher=2 $qry_con group by acc_id
                UNION ALL
                select acc_id,'account' as location,acc_trantype,'' as disctype,0 as inv_billdisc,a.acc_trandt as date,'' as inv_id,vrun_remark remark,v.vrun_work as descri,1 as qty,'' as unit,vrun_fareamt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join vehicle v on v.vrun_id = a.acc_sourceid
                left join mas_vehicle vm on vm.v_id = v.vrun_vid
                ...
                UNION ALL
                select acc_id,'account' as location,acc_trantype,'' as disctype,0 as inv_billdisc,a.acc_trandt as date,acc_sourceid as inv_id,acc_remark as remark,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ),1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                where acc_status = 1 and acc_voucher IN (3,4) $qry_con order by date asc",array_merge($qry_param,$qry_param,$qry_param,$qry_param,$qry_param));
            }

            //echo $this->db->last_query();die;

            $sl=1;$tot_dr=0;$tot_billdisc=0;$tot_cr=0;$bal=0;$bal1=0;$member='Not Selected';
            ...
            else
            {
                ...
                foreach ($ldg_data as $key => $value) 
                {   
                    if($value['inv_id']!=$bill_no)
                    {
                        $bill_no=$value['inv_id'];
                        $bill_disc=$value['inv_billdisc'];
                        $tot_billdisc += $bill_disc;
                    }
                    else
                    {
                        $bill_disc="";
                    ...
                    echo "<td class='align-right'>". $bill_disc ."</td>";
            ...
            echo "<td colspan='1' class='align-right'><strong>". $tot_billdisc ."</strong></td>";
    ...
    public function exportMemReportOld() 
    {   
        if($this->ion_auth->logged_in())
        {
            $qry_con='';
            ...
            {
                $data['ldg_data']=$this->CrudModel->select_data("
                select acc_id,acc_trantype,sl_disctype as disctype,a.acc_trandt as date,sl_invid as inv_id,inv_remark as remark,p.prod_name as descri,sl_qty as qty,p.prod_unit as unit,sl_rpu as rate,sl_gstper as gstper,sl_disc as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join s_invoice i on i.inv_id = a.acc_sourceid
                left join s_sale s on s.sl_invid= i.inv_id
                left join mas_product p on p.prod_id = s.sl_prodid
                where sl_status=1 and acc_vochfor=4 and acc_vochfor>0 and acc_voucher IN (1,2) $qry_con
                UNION ALL
                select acc_id,acc_trantype,'' as disctype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ) as descri,1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                where acc_status = 1 and acc_vochfor=4  and acc_voucher IN (3,4) $qry_con order by date asc",array_merge($qry_param,$qry_param));
            }
            if($_POST['srch_for']=='5')
            {
                $data['ldg_data']=$this->CrudModel->select_data("
                select acc_id,acc_trantype,'' as disctype,a.acc_trandt as date,'' as inv_id,vrun_remark remark,v.vrun_work as descri,1 as qty,'' as unit,vrun_fareamt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join vehicle v on v.vrun_id = a.acc_sourceid
                left join mas_vehicle vm on vm.v_id = v.vrun_vid
                where vrun_status=1 and acc_vochfor=3 and acc_voucher IN (1,2) $qry_con2
                UNION ALL
                select acc_id,acc_trantype,'' as disctype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,'Expense On Vehicle' as descri,1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                where acc_status = 1 and acc_vochfor=3 and acc_voucher=0 $qry_con3
                UNION ALL
                select acc_id,acc_trantype,'' as disctype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ) as descri,1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
            ...
            {
                $data['ldg_data']=$this->CrudModel->select_data("
                select acc_id,acc_trantype,stk_disctype as disctype,a.acc_trandt as date,stk_invid as inv_id,acc_remark as remark,p.prod_name as descri,stk_qty as qty,p.prod_unit as unit,stk_rpu as rate,stk_gstper as gstper,stk_disc as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join s_invoice i on i.inv_id = a.acc_sourceid
                left join s_stock s on s.stk_invid= i.inv_id
                left join mas_product p on p.prod_id = s.stk_prodid                 
                where stk_status=1 and acc_vochfor=1 and acc_voucher=1 $qry_con group by acc_id
                UNION ALL
                select acc_id,acc_trantype,sl_disctype as disctype,a.acc_trandt as date,sl_invid as inv_id,inv_remark as remark,p.prod_name as descri,sl_qty as qty,p.prod_unit as unit,sl_rpu as rate,sl_gstper as gstper,sl_disc as disc,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join s_invoice i on i.inv_id = a.acc_sourceid
    ...
    public function srchMemReport_old1()
    {   
        if($this->ion_auth->logged_in())
        {
            $qry_con='';
            ...
            {
                $ldg_data=$this->CrudModel->select_data("
                select acc_id,'Sale' as tran,acc_trantype,a.acc_trandt as date,inv_id,inv_remark as remark,(inv_gross+inv_gstamt-inv_disc-inv_billdisc+inv_roundoff) as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join s_invoice i on i.inv_id = a.acc_sourceid
                where inv_status=1 and acc_vochfor=4 and acc_vochfor>0 and acc_voucher IN (1,2) $qry_con
                UNION ALL
                select acc_id,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ) as as tran,acc_trantype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,a.acc_amt as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                where acc_status = 1 and acc_vochfor=4  and acc_voucher IN (3,4) $qry_con order by date desc",array_merge($qry_param,$qry_param));
            }
            if($_POST['srch_for']=='5')
            {
                $ldg_data=$this->CrudModel->select_data("
                select acc_id,v.vrun_work as tran,acc_trantype,a.acc_trandt as date,'' as inv_id,vrun_remark remark,vrun_fareamt as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join vehicle v on v.vrun_id = a.acc_sourceid
                left join mas_vehicle vm on vm.v_id = v.vrun_vid
                where vrun_status=1 and acc_vochfor=3 and acc_voucher IN (1,2) $qry_con2
                UNION ALL
                select acc_id,'Expense On Vehicle' as tran,acc_trantype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,a.acc_amt as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                where acc_status = 1 and acc_vochfor=3 and acc_voucher=0 $qry_con3
                UNION ALL
                select acc_id,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ) as tran,acc_trantype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                where acc_status = 1 and acc_vochfor=3 and acc_voucher IN (3,4) $qry_con order by date desc",array_merge($qry_param,$qry_param,$qry_param));
            }
            else
            {
                $ldg_data=$this->CrudModel->select_data("select acc_id,'Purchase Bill' as tran,acc_trantype,a.acc_trandt as date,inv_id,acc_remark as remark,(inv_gross+inv_gstamt-inv_disc-inv_billdisc+inv_roundoff) as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join s_invoice i on i.inv_id = a.acc_sourceid
                where inv_status=1 and acc_vochfor=1 and acc_voucher=1 $qry_con
                UNION ALL
                select acc_id,'Sales Bill' as tran,acc_trantype,a.acc_trandt as date,inv_id,inv_remark as remark,(inv_gross+inv_gstamt-inv_disc-inv_billdisc+inv_roundoff) as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join s_invoice i on i.inv_id = a.acc_sourceid             
                where inv_status=1 and acc_vochfor=2 and acc_voucher=2 $qry_con group by acc_id
                UNION ALL
                select acc_id,v.vrun_work as tran,acc_trantype,a.acc_trandt as date,'' as inv_id,vrun_remark remark,vrun_fareamt as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join vehicle v on v.vrun_id = a.acc_sourceid
                ...
                where acc_status = 1 and acc_vochfor=3 and acc_voucher=0 $qry_con
                UNION ALL
                select acc_id,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ) as tran,acc_trantype,a.acc_trandt as date,acc_sourceid as inv_id,acc_remark as remark,a.acc_amt as amt,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                where acc_status = 1 and acc_voucher IN (3,4) $qry_con order by date desc",array_merge($qry_param,$qry_param,$qry_param,$qry_param,$qry_param));
            }
            ...
            else
            {
                ...
                {   
                    ...
                    if($value['acc_trantype']==='1')
                    {                    
                        $tot_dr+=$dr_amt=round($value['amt'],2);
                        $bal+=round($value['amt'],2);
                    }
                    else
                    {
                        $tot_cr+=$cr_amt=round($value['amt'],2);
                        $bal-=round($value['amt'],2);
                    }
                    if($bal<0)
                        $type=" (Cr.)";
                    ...
                    echo "<td>". $sl++ ."</td>";
                    echo "<td>". date('d-m-Y',strtotime($value['date'])) ."</td>";
                    echo "<td>". $value['tran'] ."</td>";
                    echo "<td>". $value['inv_id']."</td>";
                    echo "<td>". $value['remark'] ."</td>";                    
                    echo "<td class='align-right'>". $dr_amt ."</td>";
                    echo "<td class='align-right'>". $cr_amt ."</td>";
                    echo "<td class='align-right'>". abs(round($bal,2)).$type ."</td>";
                    echo "</tr>";       
                }
    ...
    {   
        if($this->ion_auth->logged_in())
        {
            ...
            if(isset($_POST['srch_ftdate']) && $_POST['srch_ftdate']!='' && isset($_POST['srch_ttdate']) && $_POST['srch_ttdate']!=''){
                ...
                $ttdate=date('Y-m-d',strtotime($_POST['srch_ttdate']));
                $qry_con .=' and str_to_date(a.acc_trandt,"%Y-%m-%d") BETWEEN ? and ?';
                $qry_con3 .=' and str_to_date(a.acc_trandt,"%Y-%m-%d") BETWEEN ? and ?';
                array_push($qry_param,$ftdate,$ttdate);
            }
            ...
            elseif($_POST['srch_for']=='5')
            {
                ...
                left join vehicle v on v.vrun_id = a.acc_sourceid
                left join mas_vehicle mv on mv.v_id = v.vrun_vid
                where vrun_status=1 and acc_vochfor=3 and acc_voucher IN (1,2) $qry_con3
                UNION ALL
                ...
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join mas_vehicle mv on mv.v_id=a.acc_sourceid
                where acc_status =1 and acc_vochfor=3 and acc_voucher=3 $qry_con3
                UNION ALL
                select 'vehicle' as tbl,acc_id,acc_trantype,'' as disctype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,(CASE WHEN acc_voucher=3 THEN 'Amount Paid' ELSE 'Amount Received' END ) as descri,1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,0 as transport,0 as roundoff,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join vehicle v on v.vrun_id=a.acc_sourceid
                left join mas_vehicle mv on mv.v_id=v.vrun_vid
                where acc_status = 1 and acc_vochfor=3 and acc_voucher=4 $qry_con3 order by date desc",array_merge($qry_param,$qry_param,$qry_param));
            }
            else
            {
                $ldg_data=$this->CrudModel->select_data("
                ...
                UNION ALL
                select 'vehicle' as tbl,acc_id,acc_trantype,'' as disctype,a.acc_trandt as date,'' as inv_id,vrun_remark remark,v.vrun_work as descri,vrun_qty as qty,vqt_name as unit,vrun_rate as rate,'' as gstper,'' as disc,0 as transport,0 as roundoff,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a
                left join mas_members mm on mm.c_id=a.acc_cid 
                left join vehicle v on v.vrun_id = a.acc_sourceid
                left join mas_vehicle vm on vm.v_id = v.vrun_vid
                left join mas_vehicle_qtytype vqt on vqt.vqt_id = v.vrun_qtytype
                where vrun_status=1 and acc_vochfor=3 and acc_voucher IN (1,2) $qry_con
                UNION ALL
                select 'account' as tbl,acc_id,acc_trantype,'' as disctype,a.acc_trandt as date,'' as inv_id,acc_remark as remark,'Expense On Vehicle' as descri,1 as qty,'' as unit,a.acc_amt as rate,'' as gstper,'' as disc,0 as transport,0 as roundoff,CONCAT_WS(' ',mm.c_salutation,mm.c_firstname,mm.c_middlename,mm.c_lastname) as member from account a                 
                left join mas_members mm on mm.c_id=a.acc_cid 
            ...
            $data['ldg_data']=$ldg_data;
            $data['paidto']=$paidto;

            if(isset($_POST['exportbtn']))
            {
                $data['is_export']=1;
                $tot_dr=0;$tot_cr=0;$bal=0;$bal1=0;$transport_arr=[];
                foreach ($ldg_data as $key => $value) 
                {   
                    $dr_amt='';$cr_amt='';
                   if($value['disctype']==1)
                ...
                $myFile='uploads/'.$file_name.'.xls';
                //pass retrieved data into template and return as a string  
                $stringData = $this->parser->parse('reports/mem_report_srch',$data, true);
                //open excel and write string into excel  
                $fh = fopen($myFile, 'w') or die("can't open file");  
                fwrite($fh, $stringData);  
                fclose($fh);  
            ...
            else
            {
                $data['is_export']=0;
                $data['bal1']=0;
                $this->load->view('reports/mem_report_srch',$data);
            }

...
class Ion_auth_model extends CI_Model
{
	...
	public function set_session($user)
	{
		...
		$group= $this->db->select('groups.id,groups.name')
						  ->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner')
						  ->where('users_groups.user_id', $user->id)
						  ->limit(1)
						  ->order_by('id', 'asc')
						  ->get($this->tables['users_groups'])->row();
